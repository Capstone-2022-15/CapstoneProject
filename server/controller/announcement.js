...
exports.show = async (req, res) => {
  try {
    ...
    const sql =
      "SELECT A.idx, A.config_idx, A.subject, A.content, A.writer, A.writer_nick, A.createDate, A.updateDate, A.hit, A.reply FROM announcement as A WHERE A.isDeleted = 0";
...
exports.delete = async (req, res) => {
  try {
    ...
    if (rows.changedRows > 0) {
      //파일 테이블의 파일도 지운다.
      sql = "UPDATE announceFile SET isDeleted = 1 WHERE board_idx = ?";
      await connection.query(sql, params);
      //지워진 파일의 이름들을 가져온다.
      sql =
        "SELECT A.saveName FROM announceFile as A WHERE board_idx = ? AND isDeleted = 1";
...
exports.update = async (req, res) => {
  try {
    const connection = await getConnection();
    let sql =
      "UPDATE announcement SET subject = ?, content = ?, updateDate = now() WHERE idx = ? AND isDeleted = 0";
    const subject = req.body.subject;
    const content = req.body.content;
...
exports.detail = async (req, res) => {
  try {
    const connection = await getConnection();
    let sql =
      "SELECT A.idx, A.config_idx, A.subject, A.content, A.writer, A.writer_nick, A.createDate, A.updateDate, A.hit, A.reply, A.password FROM announcement as A WHERE A.isDeleted = 0 AND A.idx = ?";
    const id = req.params.id;
    ...
    if (rows[0]) {
      ...
      const data = rows[0];
      sql =
        "SELECT A.saveName FROM announceFile as A WHERE A.board_idx = ? AND A.isDeleted = 0";
...
exports.upload = async (req, res) => {
  try {
    ...
    let sql = "INSERT INTO announceFile VALUES (null,?,?,?,0)";
