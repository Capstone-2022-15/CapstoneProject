...
exports.show = async (req, res) => {
  try {
    ...
    const sql = `SELECT A.idx, A.config_idx, A.subject, A.content, A.writer, A.writer_nick, A.createDate, A.updateDate, A.hit, A.reply FROM ${category} as A WHERE A.isDeleted = 0`;
...
exports.create = async (req, res) => {
  try {
    const connection = await getConnection();
    const category = req.params.category;
    let sql = `INSERT INTO ${category} VALUES (null,?,?,?,?,?,now(),now(),0,?,0,0)`;
    const config_idx = 1;
...
exports.delete = async (req, res) => {
  try {
    ...
    let sql = `UPDATE ${category} SET isDeleted = 1 WHERE idx = ?`;
    const params = [req.params.id];
    let [rows, field] = await connection.query(sql, params);
    if (rows.changedRows > 0) {
      //파일 테이블의 파일도 지운다.
      sql = `UPDATE ${file} SET isDeleted = 1 WHERE board_idx = ?`;
      await connection.query(sql, params);
      //지워진 파일의 이름들을 가져온다.
      sql = `SELECT A.saveName FROM ${file} as A WHERE board_idx = ? AND isDeleted = 1`;
...
exports.update = async (req, res) => {
  try {
    const connection = await getConnection();
    const category = req.params.category;
    let sql = `UPDATE ${category} SET subject = ?, content = ?, updateDate = now() WHERE idx = ? AND isDeleted = 0`;
    const subject = req.body.subject;
    const content = req.body.content;
...
exports.detail = async (req, res) => {
  try {
    ...
    let sql = `SELECT A.idx, A.config_idx, A.subject, A.content, A.writer, A.writer_nick, A.createDate, A.updateDate, A.hit, A.reply, A.password FROM ${category} as A WHERE A.isDeleted = 0 AND A.idx = ?`;
    const id = req.params.id;
    let [rows, fields] = await connection.query(sql, id);
    if (rows[0]) {
      //게시글 정보
      const data = rows[0];
      sql = `SELECT A.saveName FROM ${file} as A WHERE A.board_idx = ? AND A.isDeleted = 0`;
      [rows, fields] = await connection.query(sql, id);
      //파일 정보
      const image = rows;
      res.status(200).json({ status: "200", data: data, image: image });
      //조회수 올림
      sql = `UPDATE ${category} as A set A.hit = IFNULL(hit, 0) + 1 WHERE A.idx = ?`;
...
exports.upload = async (req, res) => {
  try {
    ...
    let sql = `INSERT INTO ${file} VALUES (null,?,?,?,0)`;
