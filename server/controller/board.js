...
exports.show = async (req, res) => {
  try {
    ...
    if (rows[0]) {
      ...
      sql = `SELECT A.idx, A.config_idx, A.subject, A.content, A.writer, A.writer_nick, A.createDate, A.updateDate, A.hit, A.reply FROM board as A WHERE A.config_idx = ? AND A.isDeleted = 0`;
...
exports.update = async (req, res) => {
  try {
    ...
    if (rows[0]) {
      const config_idx = rows[0].idx;
      sql = `UPDATE board SET subject = ?, content = ?, updateDate = now() WHERE idx = ? AND isDeleted = 0 AND config_idx = ?`;
      const subject = req.body.subject;
      const content = req.body.content;
      const id = req.params.id;
      const params = [subject, content, id, config_idx];
      [rows, field] = await connection.query(sql, params);
      if (rows.changedRows > 0) {
        res.status(204).json();
      } else {
