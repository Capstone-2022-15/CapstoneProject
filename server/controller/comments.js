  ...
  try {
    ...
    switch (category) {
      case "announcement":
        ...
        break;
      case "degree":
        Comments = "degreeComments";
        break;
      case "scholarship":
        Comments = "scholarshipComments";
        break;
      case "community":
    ...
    let sql =
      `SELECT C.idx, C.member_id, C.member_nick, C.createDate, C.updateDate, C.Content FROM ${Comments} as C WHERE C.ref = ? AND C.isDeleted = 0 ORDER BY C.idx ASC`;
...
exports.create = async (req, res) => {
  try {
    ...
    let sql =
      `INSERT INTO ${Comments} VALUES (null,?,?,?,now(),?,now(),0)`;
    ...
    if (rows.affectedRows == 1) {
      //댓글 수 올리기
      sql =
        `UPDATE ${category} as A set A.reply = (SELECT COUNT(*) FROM ${Comments} as C WHERE C.ref = A.idx AND isDeleted = 0) WHERE A.idx = ?`;
...
exports.delete = async (req, res) => {
  try {
    ...
    let sql =
      `UPDATE ${Comments} as C SET C.isDeleted = 1 WHERE C.ref = ? AND C.idx = ?`;
    let id = req.params.id;
...
exports.update = async (req, res) => {
  try {
    ...
    let sql =
      `UPDATE ${Comments} as C SET C.content = ?, C.updateDate = now() WHERE C.ref = ? AND C.idx = ? AND C.isDeleted = 0`;
    let content = req.body.content;
    let id = req.params.id;
    let idx = req.params.idx;
