...
exports.delete = async (req, res) => {
  try {
    ...
    if (rows.changedRows > 0) {
      //파일 테이블의 파일도 지운다.
      sql = "UPDATE degreeFile SET isDeleted = 1 WHERE board_idx = ?";
      await connection.query(sql, params);
      //지워진 파일의 이름들을 가져온다.
      sql =
        "SELECT A.saveName FROM degreeFile as A WHERE board_idx = ? AND isDeleted = 1";
...
exports.detail = async (req, res) => {
  try {
    ...
    if (rows[0]) {
      ...
      const data = rows[0];
      sql =
        "SELECT A.saveName FROM degreeFile as A WHERE A.board_idx = ? AND A.isDeleted = 0";
      [rows, fields] = await connection.query(sql, id);
      //파일 정보
      const image = rows;
      res.status(200).json({ status: "200", data: data, image: image });
      //조회수
      sql = "UPDATE degree as A set A.hit = IFNULL(hit, 0) + 1 WHERE A.idx = ?";
...
exports.upload = async (req, res) => {
  try {
    ...
    let sql = "INSERT INTO degreeFile VALUES (null,?,?,?,0)";
