...
exports.show = async (req, res) => {
  try {
    ...
    let sql =
      "SELECT C.idx, C.member_id, C.member_nick, C.createDate, C.updateDate, C.Content FROM degreeComments as C WHERE C.ref = ? AND C.isDeleted = 0 ORDER BY C.idx ASC";
...
exports.create = async (req, res) => {
  try {
    const connection = await getConnection();
    let sql = "INSERT INTO degreeComments VALUES (null,?,?,?,now(),?,now(),0)";
    ...
    if (rows.affectedRows == 1) {
      sql =
        "UPDATE degree as A set A.reply = (SELECT COUNT(*) FROM degreeComments as C WHERE C.ref = A.idx AND isDeleted = 0) WHERE A.idx = ?";
...
exports.delete = async (req, res) => {
  try {
    const connection = await getConnection();
    let sql =
      "UPDATE degreeComments as C SET C.isDeleted = 1 WHERE C.ref = ? AND C.idx = ?";
    let id = req.params.id;
...
exports.update = async (req, res) => {
  try {
    const connection = await getConnection();
    let sql =
      "UPDATE degreeComments as C SET C.content = ?, C.updateDate = now() WHERE C.ref = ? AND C.idx = ? AND C.isDeleted = 0";
    let content = req.body.content;
    let id = req.params.id;
    let idx = req.params.idx;
