...
def main():
    ...
    image_path, appkey = '12345678-123456782.jpg', '5dd189980f4ba1c156c9a677cbe9a972'
    ...
    if 'result' in outputdata :
      for i in range(len(outputdata['result'])):
        ...
        if word.find('2022.') != -1:
          ...
          # 추출된 사진에서 정규화  YYYY-MM-DD
          re_word = re.sub('[.]', "-", word)
          #re_word = re.sub(" ", "", re_word)
          re_word1 = re.sub('[( )]', "", re_word)
          re_word2 = re.sub('[가-힣]', "", re_word1)
        
          #date_obj = datetime.strptime(re_word2, '%Y-%m-%d %H:%M:%S').date()
          
          #datetime_result = datetime.strptime(re_word3,'%Y-%m-%d')
          #print(datetime_result)
          
          result = re_word2[:8] #2022-6-9
          date = datetime.strptime(result, '%Y-%m-%d').date()
          #print(word)
          #print("----------")
          #print(result)
          print(date)
